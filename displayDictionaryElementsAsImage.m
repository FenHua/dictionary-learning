...
% display the dictiuonary atoms as blocks. For activation,the dictionary D
% should be given,as also the number of rows and columns for the atoms to
% be displayed. X and Y are the dimensions of each atom.
borderSize=1;
columnScanFlag=1;
strechEachVecFlag=1;
showImFlag=1;
...
I(:,:,1)=0; I(:,:,2)=0; I(:,:,3)=1;
% fill the image squares with the elements (in row scan or colum scan)
...
if(sortVarFlag)
   vars=var(D);
   [V,indices]=sort(vars');
   indices=fliplr(indices);% fliplr(X)Ê¹¾ØÕóXÑØ´¹Ö±Öá×óÓÒ·­×ª¡£ 
