...
%denoise an image by sparsely representing each block with the already
%overcomplete trained Dictionary,and averaging the represented parts.
...
param.errorFlag=1;
%decompose signals until a certain error is reached. do not use fix number
%of coefficients
...
for j=1:30000:size(blocks,2)
    if(waitBarOn)
        waitbar(((param.numIteration*size(blocks,2))+j)/newCounterForWaitBar);
    end
    jumpSize=min(j+30000-1,size(blocks,2));
    if(reduceDC)
        vecOfMeans=mean(blocks(:,j:jumpSize));
        blocks(:,j:jumpSize)=blocks(:,j:jumpSize)-repmat(vecOfMeans,size(blocks,1),1);
    end
    Coefs=OMPerr(Dictionary,blocks(:,j:jumpSize),errT);
    if(reduceDC)
        blocks(:,j:jumpSize)=Dictionary*Coefs+ones(size(blocks,1),1)*vecOfMeans;
    else
        blocks(:,j:jumpSize)=Dictionary*Coefs;
    end
end
count=1;
Weight=zeros(NN1,NN2);
...
IOut=(Image+0.034*sigma*IMout)./(1+0.0034*sigma*Weight);
